...
extension Client: SRWebSocketDelegate {
    ...
    public func webSocketDidOpen(_ webSocket: SRWebSocket!) {
        ...
        _ = self.sendOperationAsync(.connect(applicationId: applicationId, sessionToken: PFUser.current()?.sessionToken ?? ""))
    }
        ...
        return Task(.queue(queue)) {
            ...
            case .connected:
                self.subscriptions.forEach {
                    _ = self.sendOperationAsync(.subscribe(requestId: $0.requestId, query: $0.query, sessionToken: PFUser.current()?.sessionToken ?? ""))
                }
